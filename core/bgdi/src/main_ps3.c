...
static void ps3_add_tree( const char * dir, int depth_left )
{
    DIR * d;
    struct dirent * ent;
    struct stat st;
    ...
    while ( ( ent = readdir( d ) ) )
    {
        ...
        if ( stat( child, &st ) != 0 || !S_ISDIR( st.st_mode ) )
            continue;
        ps3_add_tree( child, depth_left - 1 );
    }
    closedir( d );
}

static void ps3_add_search_paths( const char * root )
{
    ps3_add_tree( root, 4 );
}
...
static void ps3_use_dcb( const char * dcb_path )
{
    ...
    root = file_ps3_root();
    chdir( root );
    ps3_add_search_paths( root );
    for ( i = 0 ; extra[i] ; i++ )
    {
        if ( strcmp( root, extra[i] ) != 0 )
            ps3_add_search_paths( extra[i] );
    }
    file_addp( "." );
