...
static void gr_psp_free_tile_buf( void );
...
static SDL_PixelFormat gr_psp_present_format( SDL_Surface * src )
{
     ...
     * uploads BGR565, so gr_video_present_psp converts at present time. */
...
/* Swap R/B of packed RGB565 pairs (two pixels in one uint32). */
...
static int gr_video_present_psp( SDL_Surface * src )
{
...
static int gr_psp_present_rects( SDL_Surface * src, const SDL_Rect * rects, int count )
{
    SDL_PixelFormat tex_fmt ;
    int i, total_area = 0 ;
    ...
    if ( total_area >= frame_area * 2 / 3 )
        return gr_video_present_psp( src );
...
#endif
#ifdef TARGET_PSP
    gr_psp_present_rects( src, rects, count );
    return;
#endif
...
#ifdef TARGET_PSP
    if ( !gr_psp_init_present_renderer( screen->w, screen->h ) ) return -1;
#endif
