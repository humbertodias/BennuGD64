...
void retro_get_system_info(struct retro_system_info *info)
{
   ...
   info->library_name     = "BennuGD64";
   info->library_version  = BENNUGD_VERSION[0] ? BENNUGD_VERSION : "git";
...
static void RETRO_CALLCONV retro_audio_callback(void)
{
    ...
    libretro_audio_mix(audio_mixbuf, audio_mixbuf_frames*audio_frame_size);
...
static void RETRO_CALLCONV retro_upload_audio()
{
    ...
    while(remaining_frames)
    {
        ...
        libretro_audio_mix(audio_mixbuf, chunk*audio_frame_size);
...
void retro_run(void)
{
    ...
    co_switch(bgd_thread);
    SDL_Surface* s = scale_screen ? scale_screen : screen;
    if (video_cb && s && s->pixels)
    {
        int sample_rate = libretro_audio_sample_rate();
